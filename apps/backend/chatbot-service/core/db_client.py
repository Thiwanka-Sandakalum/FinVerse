...
# Product DB
product_engine = create_engine(getattr(settings, "PRODUCT_DB_URL"), pool_pre_ping=True)
ProductSessionLocal = sessionmaker(autocommit=False, autoflush=False, bind=product_engine)

# Chat DB
chat_engine = create_engine(getattr(settings, "CHAT_DB_URL"), pool_pre_ping=True)
