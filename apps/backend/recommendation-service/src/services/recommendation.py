...
class RecommendationService:
    ...
    def __init__(self, db_service: MongoDBService):
        ...
        self.banking_service = BankingServiceClient(settings.BANKING_SERVICE_URL)
        self.model = None
        self.dataset = None
    ...
    async def _get_product_details(self, product_ids: List[str]) -> Dict[str, Dict[str, Any]]:
        """
        Get product details for the given product IDs from the banking service.
        ...
        try:
            ...
            product_details = await self.banking_service.get_product_details_batch(product_ids)
            ...
            for pid in product_ids:
                product_details[pid] = {
                    ...
                    "name": f"Product {pid}",
                    "category": "Unknown",
                    "description": "Unable to retrieve product details"
                }
            return product_details
